...
COPY --chown=root:root config/gilti.toml /etc/gilti/config.toml
...
RUN /usr/local/bin/gilti --config /etc/gilti/config.toml --check && \
    /usr/local/bin/gilti --config /etc/gilti/config.toml shell --check

EXPOSE 8080 2222
VOLUME ["/var/lib/gilti"]

ENTRYPOINT ["/usr/bin/tini", "-g", "--", "/usr/local/bin/gilti-entrypoint"]
CMD ["serve", "--config", "/etc/gilti/config.toml"]
