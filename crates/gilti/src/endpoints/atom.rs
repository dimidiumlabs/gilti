...
fn document(model: &gilti_git::history::History, path: Option<&str>, host: Option<&str>) -> String {
...
fn timestamp(identity: &gilti_git::commit::Identity) -> String {
    let Some(value) = gilti_git::time::utc(identity.timestamp) else {
...
mod tests {
    ...
    fn escapes_xml_and_identifies_sha256() {
        ...
        assert_eq!(
            super::timestamp(&gilti_git::commit::Identity {
                name: String::new(),
                email: String::new(),
                timestamp: 0,
                offset_minutes: 330
