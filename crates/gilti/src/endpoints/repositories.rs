...
impl Render for RepositoriesPage<'_> {
    fn render(&self) -> Markup {
        ...
        html! { div {
            (Header { home_url: "/", logo_url: "/-/assets/gilti.png", root_title: self.root_title, repository: None, description: self.description })
            (Tabs { items: vec![Tab { url: "/", label: "index", active: true }], trailing: Some(html! { form method="get" action="/" { input type="search" name="q" size="10" value=(self.search.unwrap_or_default()); input type="submit" value="search"; } }) })
            (ContentLayout { content, footer: html! { "generated by Gilti" } })
        } }
    }
}
...
impl RepositoriesPage<'_> {
    fn render_content(&self) -> Markup {
        html! {
            (DataTable { summary: Some("repository list"), frame: TableFrame::List { nowrap: true }, content: html! {
                ...
                @for repository in &self.repositories.rows {
                    ...
                    tr {
                        ...
                        td {
                            a class=(repositories::BUTTON) href=(&url) { "summary" }
                            @if repository.populated {
                                a class=(repositories::BUTTON) href=(format!("{url}/+/HEAD/+/log")) { "log" }
                                a class=(repositories::BUTTON) href=(format!("{url}/+/HEAD/+/tree")) { "tree" }
                            }
                        }
                    }
                }
