...
use dimidiumlabs_server::{AssetCatalog, DEFAULT_CSP, UiLayer, harden_response};
use dimidiumlabs_ui::{
    APP_SCRIPT_PATH, APP_STYLESHEET_PATH, APPLE_TOUCH_ICON_PATH, Asset, CachePolicy,
    FAVICON_ICO_PATH, FAVICON_SVG_PATH, MANIFEST_PATH, ROBOTS_PATH, css, image, javascript,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ui = axum::Router::new()
        .merge(AssetCatalog::new(application_assets())?.router::<()>())
        .route(
            "/-/licenses.json",
...
fn application_assets() -> Vec<Asset> {
    vec![
        css(APP_STYLESHEET_PATH, styles::STYLESHEET.as_bytes()),
        javascript(APP_SCRIPT_PATH, scripts::COMPONENTS.as_bytes()),
        image(
            FAVICON_ICO_PATH,
            "image/x-icon",
...
impl tower::Service<axum::extract::Request> for RepositoryService {
    ...
    fn call(&mut self, request: axum::extract::Request) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            let csp = axum::http::HeaderValue::from_static(DEFAULT_CSP);
            Ok(harden_response(method, if_none_match, response, &csp).await)
        })
    }
}

fn is_html_response(response: &axum::response::Response) -> bool {
    response
        .headers()
        .get(axum::http::header::CONTENT_TYPE)
        .is_some_and(|content_type| content_type.as_bytes().starts_with(b"text/html"))
}
...
mod tests {
    #[test]
    fn application_assets_use_shared_canonical_paths() {
        let catalog = dimidiumlabs_server::AssetCatalog::new(super::application_assets())
            .expect("valid application catalog");
        assert!(
            std::str::from_utf8(
                catalog
                    .get(dimidiumlabs_ui::APP_STYLESHEET_PATH)
        ...
        assert!(catalog.get(dimidiumlabs_ui::APP_SCRIPT_PATH).is_some());
        assert!(
            catalog
                .get(dimidiumlabs_ui::GLOBAL_STYLESHEET_PATH)
                .is_some()
        );
        assert!(catalog.get(dimidiumlabs_ui::FAVICON_ICO_PATH).is_some());
        assert!(catalog.get(dimidiumlabs_ui::FAVICON_SVG_PATH).is_some());
        assert!(
            catalog
                .get(dimidiumlabs_ui::APPLE_TOUCH_ICON_PATH)
                .is_some()
        );
        let manifest = catalog
            .get(dimidiumlabs_ui::MANIFEST_PATH)
            .expect("web manifest");
        ...
        assert!(catalog.get(dimidiumlabs_ui::ROBOTS_PATH).is_some());
    }

    #[test]
    fn html_response_detection_does_not_capture_streaming_protocols() {
        let html = axum::response::Response::builder()
            .header(axum::http::header::CONTENT_TYPE, "text/html; charset=utf-8")
            .body(axum::body::Body::empty())
            .unwrap();
        let protocol = axum::response::Response::builder()
            .header(
            ...
            .body(axum::body::Body::empty())
            .unwrap();
        assert!(super::is_html_response(&html));
        assert!(!super::is_html_response(&protocol));
    }
